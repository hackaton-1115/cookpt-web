...
export const RECIPE_THEMES: RecipeTheme[] = [
  {
    ...
    filterFn: (recipe) => {
      ...
      const hasStove = recipe.cookingTools.some((tool) => ['wok', 'pan', 'pot'].includes(tool));
      return !hasStove;
    },
  },
  {
  ...
  },
  {
    ...
    icon: Clock,
    filterFn: (recipe) =>
      recipe.prepTime + recipe.cookTime > 20 && recipe.prepTime + recipe.cookTime <= 30,
  },
  {
    id: 'vegetarian',
...
export function getRecipesByTheme(themeId: string, recipes: Recipe[]): Recipe[] {
